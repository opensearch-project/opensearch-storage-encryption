...
public class DefaultMemorySegmentPool implements MemorySegmentPool, AutoCloseable {
    private static final Logger LOGGER = LogManager.getLogger(DefaultMemorySegmentPool.class);
     ...
     */
    public DefaultMemorySegmentPool(long totalMemory, int segmentSize) {
     ...
     */
    public DefaultMemorySegmentPool(long totalMemory, int segmentSize, boolean preallocate, boolean requiresZeroing) {
    ...
    public void warmUp(int targetSegments) {
        ...
        if (allocatedSegments >= targetSegments) return;
