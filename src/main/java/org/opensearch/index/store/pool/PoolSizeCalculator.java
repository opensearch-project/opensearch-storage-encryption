...
public final class PoolSizeCalculator {
    ...
    /**
     * Gets the maximum direct memory size from JVM arguments.
     * 
     * @return the maximum direct memory size in bytes, or -1 if not set
     */
    private static long getMaxDirectMemory() {
        ...
        // If not explicitly set, JVM defaults to heap size
        return Runtime.getRuntime().maxMemory();
    }

    /**
     * Parses memory size string (e.g., "4g", "512m", "1024k") to bytes.
     * 
     * @param memorySize the memory size string
     * @return the memory size in bytes
     */
    private static long parseMemorySize(String memorySize) {
        if (memorySize == null || memorySize.isEmpty()) {
            return 0;
        }
        
        String size = memorySize.toLowerCase().trim();
        long multiplier = 1;
        ...
        try {
            ...
            LOGGER.warn("Failed to parse memory size: {}, defaulting to 0", memorySize);
            return 0;
        }
    }
     ...
     *
     * pool_size = (totalPhysicalMemory - maxHeap - maxDirectMemory) * pool_size_percentage
     *
     * @param settings the node settings for configuration
    ...
    public static long calculatePoolSize(Settings settings) {
        ...
        // Subtract both heap and direct memory from total physical memory
        long offHeap = Math.max(0, totalPhysical - maxHeap - maxDirectMemory);
        ...
        LOGGER
            .info(
                String
                    .format(
                        Locale.ROOT,
                        "Calculated pool size = %d MB (%.1f GB) [total=%.1f GB, heap=%.1f GB, direct=%.1f GB, offheap=%.1f GB, percentage=%.1f%%]",
